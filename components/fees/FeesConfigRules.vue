...
const feeOptions = computed(() =>
    fees.value.map(fee => ({ label: fee.name, value: fee.id }))
)
...
<template>
    <div class="space-y-4">
        ...
        <!-- Liste des règles -->
        <UTable :rows="filteredRules" :columns="[
            { key: 'fee', label: 'Frais' },
            ...
            <template #applies_to_type-data="{ row }">
                <UBadge color="blue" variant="subtle">{{ row.applies_to_type }}</UBadge>
            </template>

            <template #range-data="{ row }">
                <span v-if="row.min_value || row.max_value" class="text-sm">
        ...
        <UModal v-model="showModal">
            <UCard>
                ...
                <UForm :state="currentRule" :schema="feeRuleSchema" @submit="saveRule" class="space-y-4">
                    <UFormGroup label="Frais" name="fee_id" required>
                        <USelectMenu
                    ...
                    <UFormGroup label="S'applique à" name="applies_to_type" required>
                        <USelectMenu
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <UFormGroup label="Valeur minimale" name="min_value">
                            <UInput
                        ...
                        <UFormGroup label="Valeur maximale" name="max_value">
                            <UInput
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <UFormGroup label="Montant (XAF)" name="amount">
                            <UInput
                        ...
                        <UFormGroup label="Pourcentage (%)" name="percentage">
                            <UInput
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <UButton type="submit" :loading="feeStore.loading">
