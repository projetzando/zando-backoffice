...
<template>
    <div class="space-y-4">
        ...
        <!-- Liste des frais -->
        <UTable :rows="filteredFees" :columns="[
            { key: 'name', label: 'Nom' },
            ...
            <template #type-data="{ row }">
                <UBadge :color="row.is_percentage ? 'blue' : 'green'" variant="subtle">
        ...
        <UModal v-model="showModal">
            <UCard>
                ...
                <UForm :state="currentFee" :schema="feeSchema" @submit="saveFee" class="space-y-4">
                    <UFormGroup label="Nom" name="name" required>
                        <UInput v-model="currentFee.name" placeholder="Ex: Frais de service" />
                    </UFormGroup>

                    <UFormGroup label="Libellé" name="label">
                        <UInput v-model="currentFee.label" placeholder="Ex: Service fee" />
                    </UFormGroup>
                    ...
                    <UFormGroup label="Type" name="is_percentage">
                        <URadioGroup
