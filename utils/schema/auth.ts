  ...
  .object({
    ...
    password: z
      ...
      .refine(value => !value.toLowerCase().includes('password'), {
        message: 'Le mot de passe ne peut pas contenir le mot "password"',
      }),
    ...
    password_confirmation: z
      ...
      .regex(
        new RegExp('.*[A-Z].*'),
        'Le mot de passe de confirmation doit contenir au moins une lettre majuscule',
      )
      .regex(
        new RegExp('.*[a-z].*'),
        'Le mot de passe de confirmation doit contenir au moins une lettre majuscule',
      ...
      .regex(
        new RegExp('.*[0-9].*'),
        'Le mot de passe de confirmation doit contenir au moins un chiffre',
      )
      .regex(
        new RegExp('.*[`~<>?,./!@#$%^&*()\\-_+="\'|{}\\[\\];:\\\\].*'),
        'Le mot de passe de confirmation doit contenir au moins un caractère special',
      )
      .refine(value => !value.toLowerCase().includes('password'), {
        message: 'Le mot de passe de confirmation ne peut pas contenir le mot "password"',
      }),
  })
