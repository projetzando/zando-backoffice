...
const tokenCookie = useCookie<string>('zando_token', {
    httpOnly: false,
    secure: true,
    sameSite: 'strict',
})
    ...
    <FormAuth title="Connexion" :errors="display_errors" :error="display_error">
        <UForm :state="user" :schema="authSchema" class="space-y-4 mt-6" @submit="Login">
            ...
            <div class="text-right">
                <NuxtLink to="/" class="text-sm font-semibold text-gray-700 hover:text-blue-700 focus:text-blue-700">Mot
                    de passe
                    oublié?</NuxtLink>
            </div>

            <UButton type="submit" :loading="authStore.loading" label="Se connecter"
