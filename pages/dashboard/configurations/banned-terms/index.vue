...
<template>
    <div class="space-y-6">
        <!-- Header -->
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Termes bannis</h1>
            <p class="text-gray-600 mt-1">Gérez les termes et patterns interdits dans les conversations</p>
        </div>
        ...
        <!-- Liste des termes -->
        <UTable :rows="filteredTerms" :columns="[
            ...
            <template #term-data="{ row }">
                <div>
                    <p class="font-medium font-mono">{{ row.term }}</p>
                    <p v-if="row.custom_alert_message" class="text-xs text-gray-500">
            ...
            <template #category-data="{ row }">
                <UBadge color="blue" variant="subtle">{{ getCategoryLabel(row.category) }}</UBadge>
            </template>

            <template #is_regex-data="{ row }">
                <UBadge :color="row.is_regex ? 'purple' : 'gray'" variant="subtle">
                    {{ row.is_regex ? 'Regex' : 'Texte' }}
                </UBadge>
            </template>

            <template #severity-data="{ row }">
                <UBadge :color="getSeverityColor(row.severity)" variant="subtle">
                    {{ getSeverityLabel(row.severity) }}
                </UBadge>
            </template>

            <template #description-data="{ row }">
                <span v-if="row.description" class="text-sm">{{ row.description }}</span>
        ...
        <UModal v-model="showModal">
            <UCard>
                ...
                <UForm :state="currentTerm" :schema="bannedTermSchema" @submit="saveTerm" class="space-y-4">
                    ...
                    <UFormGroup label="Terme ou pattern" name="term" required>
                        <UInput
                    ...
                    <UFormGroup label="Type" name="is_regex">
                        <URadioGroup
                    ...
                    <UFormGroup label="Sévérité" name="severity" required>
                        <USelectMenu
                    ...
                    <UFormGroup label="Message d'alerte personnalisé" name="custom_alert_message">
                        <UInput
                    ...
                    <UFormGroup label="Actif" name="is_active">
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <UButton type="submit" :loading="bannedTermStore.loading">
