...
async function updateAlertStatus(alert: Alert, newStatus: Alert['status']) {
  const result = await alertStore.updateStatus(alert.id!, newStatus, authStore.connected_user?.id)
  ...
  <div class="space-y-6">
    ...
    <div>
      ...
      <p class="text-gray-600 mt-1">
        Gérez les alertes détectées dans les conversations
      </p>
    </div>
    ...
    >
      ...
      <template #sender-data="{ row }">
        <div>
          ...
          <p class="text-xs text-gray-500">
            ID: {{ row.sender_id?.substring(0, 8) }}...
          </p>
        </div>
      </template>
      ...
      <template #detected_content-data="{ row }">
        <p
          v-if="row.detected_content"
          class="text-sm text-red-600 truncate max-w-xs"
        >
