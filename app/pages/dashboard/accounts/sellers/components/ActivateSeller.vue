...
function validate() {
    ...
    sellerStore.activate(seller.value?.id).then(async (data) => {
        closeModal()
        ...
        <UCard>
            <template #header>
                <div class="flex items-center justify-between">
                    <h1> {{ seller?.etat ? "Désactivation de l'établissement" : "Activation de l'établissement"
                        }}</h1>
            ...
            <template #default>
                <p>Êtes-vous sûr de vouloir {{ seller?.etat ? "désactiver" : "activer" }} le vendeur
